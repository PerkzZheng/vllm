...
_QSA_PRIMS_TS_AUTO_DEVICE_CAPABILITIES = (103,)
...
def _is_qsa_prims_ts_auto_qualified_device() -> bool:
    """Return whether this architecture has completed vLLM runtime qualification."""

    return any(
        current_platform.is_device_capability(capability)
        for capability in _QSA_PRIMS_TS_AUTO_DEVICE_CAPABILITIES
    )
...
def _resolve_qsa_prims_ts_backend(
    *,
    ...
    if backend == "triton":
        return False
    selected_capable = capable and (backend == "prims_ts" or auto_qualified)
    available = availability_probe() if selected_capable else False
    supported = selected_capable and available
    ...
    def __init__(
        ...
        *,
        ...
        self.use_qsa_prims_ts = _resolve_qsa_prims_ts_backend(
            ...
            auto_qualified=_is_qsa_prims_ts_auto_qualified_device(),
