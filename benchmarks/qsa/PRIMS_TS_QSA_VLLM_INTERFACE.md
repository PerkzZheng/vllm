...
The [validation report](VALIDATION_20260909.md) records pre-rebase correctness,
...
The Triton path uses the same upstream indexer, then expands its block IDs to
token IDs. Current main appends each row's valid-entry count in a separate
last column. That count is not a token ID and is not part of PrimTS's compact
input. Both backends reuse their entire selection buffer during later MTP
drafting steps.
For compact MTP selections, a small position buffer preserves the step-0 tail
boundary and is compacted with the block-ID rows. Target layers do not allocate
or copy this buffer.
