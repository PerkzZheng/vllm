...
qsa_script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)
qsa_repo=${QSA_REPO:-$(cd -- "${qsa_script_dir}/../.." && pwd -P)}
...
# Use the selected source checkout and its matching compiled extensions for
# both the server and benchmark client. Keep the FlashInfer image overlay
# identical across every action so a
