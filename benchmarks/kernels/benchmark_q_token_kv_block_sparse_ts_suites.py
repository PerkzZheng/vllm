...
def _prepare_case_tensors(
    ...
    expanded_indices = torch.empty(
        (rows, EXPANDED_WIDTH + 1), dtype=torch.int32, device="cuda"
    )
    groups = (
    ...
    return CaseTensors(
        ...
        visible_blocks=torch.minimum(
            (query_positions + 1) // SPARSE_BLOCK_SIZE,
            sequence_lengths.index_select(0, token_to_request.long())
            // SPARSE_BLOCK_SIZE,
        ).to(torch.int32),
...
def _expected_expanded_indices(tensors: CaseTensors) -> torch.Tensor:
    indices = _expand_compact_indices(
        tensors.block_indices,
        tensors.tail_token_indices,
        tensors.query_positions,
    )
    counts = (indices >= 0).sum(dim=1, keepdim=True).to(torch.int32)
    return torch.cat((indices, counts), dim=1)
...
def _resolve_triton_config(
    tensors: CaseTensors, use_prefill_config: bool
) -> dict[str, int]:
    """Report the checked-in Triton policy without duplicating its thresholds."""

    q = tensors.query_flat
    num_kv_heads = int(tensors.triton_k.shape[2])
    heads_per_kv = int(q.shape[1]) // num_kv_heads
    block_n, partial_warps, _, num_splits = _select_config(
        int(q.shape[0]), num_kv_heads, use_prefill_config, EXPANDED_WIDTH
    )
    return {
        "block_m": 1 << (heads_per_kv - 1).bit_length(),
        "block_n": block_n,
        "partial_warps": partial_warps,
        "splits_kv": num_splits,
    }
...
def _sample_fp32_oracle(
    ...
    *,
    ...
    for row in sampled_rows:
        logical = tensors.expanded_indices[row, :EXPANDED_WIDTH]
    ...
    *,
    ...
    def triton_attention() -> object:
        return qsa_sparse_paged_attention(
            ...
            tensors.block_table,
            tensors.token_to_request,
            use_prefill_config=case.phase == "prefill",
            out=tensors.triton_output,
            bmm1_scale=HEAD_DIM**-0.5,
        )
    ...
    triton_config = _resolve_triton_config(tensors, case.phase == "prefill")
